...
export function matchToolPattern(pattern: string, toolName: string): boolean {
  ...
  if (toolName !== toolPart) return false;
...
export async function executeHooks(
  ...
  const matchingHooks = options.toolName
    ? hooks.filter(
        (h) => !h.matcher || matchToolPattern(h.matcher, options.toolName!)
      )
    : hooks;
 ...
 */
export function hasHooksForEvent(event: HookEvent): boolean {
  try {
 ...
 */
export function hasHooksForTool(
  event: HookEvent,
  toolName: string
): boolean {
  try {
    const hooks = getHooksFromSnapshot(event);
    return hooks.some(
      (h) => !h.matcher || matchToolPattern(h.matcher, toolName)
    );
  } catch {
    return false;
  }
