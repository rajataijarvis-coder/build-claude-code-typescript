...
import { Tool, ToolCall, ToolUseContext } from './types.js';
...
export class StreamingToolExecutor {
   ...
   */
  addTool(block: { id: string; name: string; input: Record<string, unknown> }, assistantMessage: { content: string }): void {
   ...
   */
  *getCompletedResults(): Generator<{ toolId: string; result: { tool_call_id: string; content: string; isError?: boolean } }> {
    if (this.discarded) return;
   ...
   */
  async *getRemainingResults(): AsyncGenerator<{ toolId: string; result: { tool_call_id: string; content: string; isError?: boolean } }> {
    if (this.discarded) return;
