 ...
 */
export async function scanSkills(sources) {
            ...
            for (const entry of entries) {
                ...
                if (skills.has(skillName)) {
                    ...
                    if (source.priority >= existing.source.priority) {
                        console.log(`Skipping ${skillName} from ${source.name} (lower priority)`);
                        continue;
                    }
                }
 ...
 */
export function formatSkillsForSystemPrompt(index) {
 ...
 */
export function getSkillByName(index, name) {
 ...
 */
export function filterSkillsByTag(index, tag) {
    return Array.from(index.skills.values()).filter((skill) => skill.frontmatter.tags?.includes(tag));
}
/**
 * Count skills by source.
 */
export function countSkillsBySource(index) {
