 ...
 */
export async function executeToolPipeline(toolCall, context, registry) {
    // === STEP 1: Tool Lookup ===
    ...
    const hookResult = { behavior: 'passthrough' };
    ...
    const permission = await resolvePermission(tool, enrichedInput, context, hookResult, classifierPromise);
 ...
 */
function enrichInput(input, context) {
    ...
    if ('file_path' in enriched && typeof enriched.file_path === 'string') {
        const filePath = enriched.file_path;
        if (filePath.startsWith('~/')) {
            enriched.file_path = filePath.replace('~/', homedir() + '/');
        }
 ...
 */
async function applyResultBudgeting(result, maxSize, _context) {
    if (maxSize === Infinity) {
        return result;
    }
 ...
 */
function handleToolError(_tool, toolCall, error, _context) {
