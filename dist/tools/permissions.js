 ...
 */
export async function resolvePermission(tool, input, context, hookResult, classifierPromise) {
    ...
    const toolCheck = tool.checkPermissions(input, context);
    ...
    if (mode.denyWriteOperations && !tool.isReadOnly(input)) {
    ...
    if (mode.autoAllowToolPatterns) {
        const shouldAutoAllow = mode.autoAllowToolPatterns.some(pattern => tool.name.includes(pattern));
        if (shouldAutoAllow) {
 ...
 */
function matchPermissionRule(toolName, input, context) {
    const allRules = [
