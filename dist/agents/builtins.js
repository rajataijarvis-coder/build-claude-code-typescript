 ...
 */
function createGeneralPurposeAgent() {
    return {
        ...
        background: false,
        omitClaudeMd: false,
        getSystemPrompt: (ctx) => `You are a Claude Code sub-agent. Complete the task fully.
...
function createExploreAgent() {
    return {
        ...
        omitClaudeMd: true,
        color: 'green',
        getSystemPrompt: (ctx) => `=== CRITICAL: READ-ONLY MODE ===
...
function createPlanAgent() {
    return {
        ...
        omitClaudeMd: true,
        color: 'blue',
        getSystemPrompt: (ctx) => `You are a software architect. Your job is to DESIGN, not IMPLEMENT.
...
function createVerificationAgent() {
    return {
        ...
        omitClaudeMd: false,
        color: 'red',
        getSystemPrompt: (ctx) => `You are a VERIFICATION agent. Find problems, don't fix them.
...
function createGuideAgent() {
    return {
        ...
        background: false,
        omitClaudeMd: false,
        getSystemPrompt: (ctx) => `You are the Claude Code documentation agent.
 ...
 * Statusline Agent
 */
function createStatuslineAgent() {
    return {
        ...
        background: false,
        color: 'orange',
        getSystemPrompt: (ctx) => `You are the statusline configuration agent.
