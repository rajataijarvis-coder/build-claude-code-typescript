...
export class AsyncAgentManager {
     ...
     */
    async launch(agentDefinition, prompt, parentContext, agentId, outputFile) {
    ...
    async complete(agentId, result) {
        ...
        agent.result = result;
        await writeFile(agent.outputFile, JSON.stringify({
            agentId,
            status: 'completed',
            startTime: agent.startTime,
 ...
 */
export function createBackgroundSignal(timeoutMs = 120000) {
 ...
 */
export function isAutoBackgroundEnabled() {
 ...
 */
export function getAutoBackgroundTimeout() {
 ...
 */
export async function* runWithAutoBackground(agentGenerator, transition) {
    ...
    try {
        while (true) {
            ...
            if (result === BACKGROUND_SIGNAL) {
                ...
                await iterator.return?.({ status: 'async_launched' });
 ...
 */
export function registerForegroundAgent(agentId, initialMessages) {
    const transcriptFile = join(homedir(), '.claude', 'agents', `${agentId}-transcript.json`);
    return {
