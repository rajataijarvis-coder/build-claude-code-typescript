 ...
 */
export async function loadUserAgents(projectRoot) {
    ...
    try {
        ...
        for (const entry of entries) {
            if (entry.isFile() && entry.name.endsWith('.md')) {
                const agent = await loadAgentFromFile(join(agentsDir, entry.name), 'user');
                if (agent) {
                    agents.push(agent);
                }
            }
 ...
 */
async function loadAgentFromFile(filePath, source) {
    try {
