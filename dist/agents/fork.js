 ...
 */
export function wrapDirective(directive) {
 ...
 */
export function buildForkedMessages(parentAssistantMessage, directive) {
    ...
    const placeholderResults = parentAssistantMessage.tool_calls?.map((toolCall) => ({
        role: 'tool',
 ...
 */
export function isForkChild(context) {
 ...
 */
export function preventRecursiveFork(context, requestedType) {
...
export function filterIncompleteToolCalls(messages) {
    ...
    return result.map(msg => {
        if (msg.role === 'assistant' && msg.tool_calls) {
            const completeCalls = msg.tool_calls.filter((tc) => !pendingToolUses.has(tc.id));
 ...
 */
export async function* spawnForkAgent(params) {
    const { parentContext, forkContextMessages, parentSystemPrompt, parentTools, directive, agentId, } = params;
    // Build the messages for this fork child
    const lastMessage = forkContextMessages[forkContextMessages.length - 1];
    const [clonedAssistant, userMessage] = buildForkedMessages(lastMessage, directive);
 ...
 */
export function calculateCacheSavings(sharedPrefixSize, childCount) {
    if (childCount <= 1)
        return 0;
